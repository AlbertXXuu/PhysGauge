...
def run_smoke_calibration() -> dict[str, Any]:
    result = run_suite(SuiteConfig(cases=4, frames=16, seed=7))
  ...
  <div class="page-frame">
    <header class="site-header liquid-surface">
      ...
      <nav aria-label="Evidence views">
        <button class="nav-button active" type="button" data-view="calibration">Calibration</button>
        <button class="nav-button" type="button" data-view="learned">Learned-model study</button>
      </nav>
    ...
    <main id="top">
      ...
      <section class="view-panel" id="learned-view" data-panel="learned" hidden>
        <header class="section-heading">
...
function show(name){$$('[data-panel]').forEach(p=>{const active=p.dataset.panel===name;p.hidden=!active;p.classList.toggle('active',active)});$$('[data-view]').forEach(b=>b.classList.toggle('active',b.dataset.view===name));if(name==='learned')$('#learned-view').scrollIntoView({behavior:'smooth'});}
function renderCalibration(d){$('#metric-cases').textContent=d.caseCount;$('#metric-candidates').textContent=d.candidateCount;$('#metric-records').textContent=d.recordCount;$('#metric-oracles').textContent=d.oraclesValid&&d.violationsDetected?'100%':'Check';$('#calibration-protocol').textContent=d.protocol;$('#calibration-source').textContent=d.source;const headers=['Candidate','Physics caught',...Object.values(d.metricLabels)];let html=headers.map(h=>`<div class="matrix-cell heading">${h}</div>`).join('');for(const row of d.rows){html+=`<div class="matrix-cell name">${row.name}</div><div class="matrix-cell value" style="--value:${row.physicsDetection}">${pct(row.physicsDetection)}</div>`;for(const key of Object.keys(d.metricLabels)){const v=row.lowSensitivity[key];html+=`<div class="matrix-cell value" style="--value:${v}">${pct(v)}</div>`;}}$('#sensitivity-matrix').innerHTML=html;}
...
async function run(){const button=$('#run-smoke'),status=$('#run-status');button.disabled=true;status.textContent='Running 4 seeded cases and 40 candidate trajectories…';try{const response=await fetch('/api/smoke',{method:'POST'});const data=await response.json();if(!response.ok)throw new Error(data.error||'Smoke check failed');renderCalibration(data);status.textContent=`PASS · ${data.recordCount} fresh records · all oracle contracts and injected violations verified`;$('#oracle-status').textContent='Fresh pass';show('calibration');$('#calibration-view').scrollIntoView({behavior:'smooth'});}catch(error){status.textContent=`Unable to run: ${error.message}`;}finally{button.disabled=false;}}
window.addEventListener('DOMContentLoaded',()=>{load().catch(error=>{$('#run-status').textContent=error.message});$$('[data-view]').forEach(button=>button.addEventListener('click',()=>show(button.dataset.view)));$('#run-smoke').addEventListener('click',run);});
"""
...
class _StudioHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:  # noqa: N802 - BaseHTTPRequestHandler API
        ...
        if not self.server.run_lock.acquire(blocking=False):
            ...
            return
        try:
            self._json(run_smoke_calibration())
