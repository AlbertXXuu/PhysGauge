            ...
            (
                {
                    ...
                    "value": float(entry["mean_metrics"][metric]),
                }
                for name, entry in candidates.items()
                if entry["family"] == RESPONSE_FAMILY
            ),
            key=lambda point: point["severity"],
        )
        response_series.append(
            {"metric": metric, "metricLabel": METRIC_LABELS[metric], "points": points}
        )
    return {
        "source": source,
        ...
        "severityResponse": {
            ...
            "severityLabel": "Injected energy-loss severity",
            "aggregation": f"Mean over {int(result['config']['cases'])} seeded cases",
            "normalization": "Each metric divided by its maximum measured response",
...
<body>
  <div class="page-frame">
    <header class="site-header liquid-surface">
      <button class="brand-link" type="button" data-alvenx-home aria-label="AlvenX — Back to top">
      ...
      <details class="view-menu" data-view-menu>
        <summary aria-label="Choose evidence view">Views <span aria-hidden="true">⌄</span></summary>
        <div class="view-menu__panel" aria-label="Evidence views">
          <button type="button" data-view="calibration" aria-current="true">Calibration</button>
          <button type="button" data-view="learned" aria-current="false">Learned-model study</button>
        </div>
      </details>
      <span class="local-badge"><i aria-hidden="true"></i>Studio v1.1.2 · Calibration evidence v1.0.0</span>
    </header>
    ...
    <main id="top">
        ...
        <div class="hero-copy">
          ...
          <div class="hero-actions">
            <button class="liquid-button" id="run-smoke" type="button">
              <span class="link-label">Run local smoke check</span><span class="link-arrow" aria-hidden="true">↗</span>
            </button>
            ...
            <button class="text-button" type="button" data-view="learned"><span class="link-label">Open learned-model study</span><span class="link-arrow" aria-hidden="true">↓</span></button>
          </div>
          <p class="run-status" id="run-status" role="status" aria-live="polite">
            4 seeded cases · 40 candidate trajectories · in-memory result
          </p>
        </div>
        <aside class="hero-visual" aria-labelledby="response-title">
          <div class="visual-label"><span>FROZEN MEASUREMENTS</span><strong id="oracle-status">Oracle validated</strong></div>
          <div class="response-heading">
            <div><span id="response-family">Inelastic family</span><h2 id="response-title">Severity → metric response</h2></div>
            <span id="response-scope">4 metrics · 24-case means</span>
          </div>
          <div class="response-plot" id="response-plot" aria-live="polite">
            <p>Loading measured response…</p>
          </div>
          <p class="visual-explainer" id="response-caption">Four metrics across three measured severity levels. Each curve uses its own observed maximum; lines connect observations only.</p>
        </aside>
      </section>
      ...
      <section class="view-panel" id="learned-view" data-panel="learned"
        ...
        <details class="protocol-details">
          ...
          <p><strong>R2 · research milestone:</strong> R2 means research milestone 2—learned-model
...
.matrix-cell.value.v0{--matrix-alpha:0%}.matrix-cell.value.v1{--matrix-alpha:1.6%}.matrix-cell.value.v2{--matrix-alpha:3.2%}.matrix-cell.value.v3{--matrix-alpha:4.8%}.matrix-cell.value.v4{--matrix-alpha:6.4%}.matrix-cell.value.v5{--matrix-alpha:8%}.matrix-cell.value.v6{--matrix-alpha:9.6%}.matrix-cell.value.v7{--matrix-alpha:11.2%}.matrix-cell.value.v8{--matrix-alpha:12.8%}.matrix-cell.value.v9{--matrix-alpha:14.4%}.matrix-cell.value.v10{--matrix-alpha:16%}
...
body{min-width:0}.brand-link{min-height:44px;align-items:center;flex:0 0 auto}.nav-button{min-height:44px;transition:background-color 300ms var(--ease),color 300ms var(--ease),box-shadow 300ms var(--ease)}.view-menu{position:relative;display:none;margin-left:auto}.view-menu summary{display:flex;min-width:64px;min-height:44px;align-items:center;justify-content:center;gap:5px;padding:0 10px;border:1px solid transparent;border-radius:999px;background:transparent;color:var(--muted);font:620 .7rem/1 "Instrument Sans";letter-spacing:.05em;text-transform:uppercase;cursor:pointer;list-style:none;transition:background-color 300ms var(--ease),border-color 300ms var(--ease),color 300ms var(--ease)}.view-menu summary::-webkit-details-marker{display:none}.view-menu summary:hover,.view-menu[open] summary{border-color:rgb(255 255 255 / 82%);background:rgb(255 255 255 / 35%);color:var(--primary)}.view-menu summary:focus-visible,.view-menu__panel button:focus-visible{outline:3px solid rgb(37 99 235 / 45%);outline-offset:4px}.view-menu__panel{position:absolute;top:calc(100% + 12px);right:0;display:grid;min-width:210px;gap:4px;padding:6px;border:1px solid var(--edge);border-radius:18px;background:rgb(251 253 255 / 92%);box-shadow:inset 0 1px 0 var(--highlight),0 18px 50px rgb(71 105 148 / 16%);-webkit-backdrop-filter:blur(18px) saturate(148%);backdrop-filter:blur(18px) saturate(148%)}.view-menu__panel button{min-height:44px;padding:9px 12px;border:1px solid transparent;border-radius:12px;background:transparent;color:var(--muted);font:620 .76rem/1.2 "Instrument Sans";text-align:left;cursor:pointer}.view-menu__panel button:hover{border-color:rgb(255 255 255 / 82%);background:rgb(255 255 255 / 35%);color:var(--primary)}.view-menu__panel button[aria-current="true"]{background:rgb(255 255 255 / 72%);color:var(--primary)}.liquid-button{transition:transform 420ms var(--ease),border-color 420ms var(--ease),background-color 420ms var(--ease),box-shadow 420ms var(--ease)}.liquid-button,.text-button{text-decoration:none}.text-button{display:inline-flex;min-height:44px;align-items:center;gap:.35em;padding:8px 4px}.text-button .link-label{text-decoration:underline;text-underline-offset:.14em}.link-arrow{display:inline-block;flex:0 0 auto;text-decoration:none}.liquid-button .link-arrow{font-weight:700}.liquid-button:focus-visible,.text-button:focus-visible,.nav-button:focus-visible,.brand-link:focus-visible,a:focus-visible{outline:3px solid rgb(37 99 235 / 45%)!important;outline-offset:4px!important}.hero-visual{min-width:0}.response-heading{display:flex;align-items:flex-end;justify-content:space-between;gap:20px;margin:28px 0 14px}.response-heading span{color:var(--muted);font-size:.72rem}.response-heading h2{margin:4px 0 0;font-size:clamp(1.45rem,2.4vw,2.25rem);font-weight:570;letter-spacing:-.025em}.response-plot{min-height:270px;padding:12px;border-radius:20px;background:linear-gradient(145deg,rgb(255 255 255 / 45%),rgb(147 197 253 / 20%));color:var(--muted)}.response-plot svg{display:block;width:100%;height:auto;overflow:visible}.response-grid{stroke:rgb(71 105 148 / 15%);stroke-width:1}.response-axis{stroke:rgb(71 105 148 / 32%);stroke-width:1.2}.response-line{fill:none;stroke:var(--violet);stroke-width:4;stroke-linecap:round;stroke-linejoin:round}.response-dot{fill:white;stroke:var(--violet);stroke-width:4}.response-value{fill:var(--primary);font:650 12px "Instrument Sans"}.response-tick{fill:var(--muted);font:600 11px "Instrument Sans"}.response-points{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:8px;margin-top:12px}.response-points span{display:grid;gap:2px;padding:8px 10px;border-radius:12px;background:rgb(255 255 255 / 52%);color:var(--muted);font-size:.68rem;overflow-wrap:anywhere}.response-points strong{color:var(--primary);font-size:.78rem}.visual-explainer{margin:18px 0 0;color:var(--reading);font-size:.82rem;line-height:1.55}
.response-line.s0{stroke:#2563eb}.response-line.s1{stroke:#4f46e5;stroke-dasharray:24 12}.response-line.s2{stroke:#7c3aed;stroke-dasharray:4 12}.response-line.s3{stroke:#3b82f6;stroke-dasharray:24 12 4 12}.response-dot.s0{stroke:#2563eb}.response-dot.s1{stroke:#4f46e5}.response-dot.s2{stroke:#7c3aed}.response-dot.s3{stroke:#3b82f6}.response-points{grid-template-columns:repeat(2,minmax(0,1fr))}.response-points strong{display:flex;align-items:center;gap:7px}.response-points strong i{width:18px;height:3px;border-radius:999px;background:#2563eb}.response-points strong i.s1{background:#4f46e5}.response-points strong i.s2{background:#7c3aed}.response-points strong i.s3{background:#3b82f6}.response-points small{color:var(--muted);font-size:.62rem;line-height:1.45}
@media(min-width:1101px){.hero{grid-template-columns:minmax(0,1.3fr) minmax(400px,.7fr);gap:clamp(40px,5vw,76px)}}
@media(max-width:1100px){.local-badge{display:none}.hero{min-height:auto;grid-template-columns:minmax(0,1fr);gap:44px;padding:140px 0 72px}.hero-copy{max-width:880px}.hero-visual{width:100%}.metric-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.metric-grid article:nth-child(2){border-right:0}.study-summary{grid-template-columns:1fr}.section-heading,.matrix-wrap>header{align-items:flex-start;flex-direction:column}.section-heading code{max-width:100%}}
@media(max-width:760px){.site-header nav{display:none}.view-menu{display:block}.hero h1{font-size:clamp(2.7rem,12vw,4.7rem)}.metric-grid,.seed-grid{grid-template-columns:1fr}.metric-grid article{border-right:0;border-bottom:1px solid rgb(71 105 148 / 18%)}.metric-grid article:last-child{border-bottom:0}.response-heading{align-items:flex-start;flex-direction:column}.response-points{grid-template-columns:1fr}.matrix-wrap{overflow-x:auto}}
@media(max-width:360px){.site-header{gap:16px}.view-menu summary{min-width:44px;padding-inline:6px}.view-menu summary span{display:none}}
@media(prefers-reduced-motion:reduce){html{scroll-behavior:auto}.liquid-button,.liquid-button::before,.nav-button,.view-menu summary{transition:none}}
"""


STUDIO_JS = r"""const $=s=>document.querySelector(s),$$=s=>document.querySelectorAll(s),pct=v=>`${Math.round(v*100)}%`;
function backToTop(){if(window.scrollY===0)return;window.scrollTo({top:0,left:window.scrollX,behavior:window.matchMedia('(prefers-reduced-motion: reduce)').matches?'instant':'smooth'});}
function show(name,{scroll=true,updateHash=true}={}){const target=$(`#${name}-view`);if(!target)return;$$('[data-panel]').forEach(panel=>{const active=panel===target;panel.hidden=!active;panel.classList.toggle('active',active);if(active){panel.classList.add('view-enter');requestAnimationFrame(()=>requestAnimationFrame(()=>panel.classList.remove('view-enter')));}});$$('[data-view]').forEach(button=>{const active=button.dataset.view===name;if(button.matches('[role="tab"]')){button.classList.toggle('active',active);button.setAttribute('aria-selected',String(active));}else{button.setAttribute('aria-current',String(active));}});$('[data-view-menu]').removeAttribute('open');if(updateHash)history.replaceState(null,'',`#${name}-view`);if(scroll)target.scrollIntoView({behavior:'smooth',block:'start'});}
...
function renderResponse(r){const raw=v=>v<.001?v.toExponential(2):v.toFixed(4),ordered=r.series.map(s=>({...s,points:[...s.points].sort((a,b)=>a.severity-b.severity)})),maxSeverity=Math.max(...ordered[0].points.map(p=>p.severity)),series=ordered.map((s,i)=>{const maxValue=Math.max(...s.points.map(p=>p.value)),coords=s.points.map(p=>({x:48+(p.severity/maxSeverity)*270,y:166-(p.value/maxValue)*116,...p})),line=coords.map(p=>`${p.x.toFixed(1)},${p.y.toFixed(1)}`).join(' '),dots=coords.map(p=>`<circle class="response-dot s${i}" cx="${p.x}" cy="${p.y}" r="5"><title>${s.metricLabel} · ${pct(p.severity)} severity · ${raw(p.value)}</title></circle>`).join('');return {markup:`<polyline class="response-line s${i}" points="${line}"></polyline>${dots}`,legend:`<span><strong><i class="s${i}"></i>${s.metricLabel}</strong><small>${s.points.map(p=>`${pct(p.severity)} ${raw(p.value)}`).join(' · ')}</small></span>`};}),ticks=ordered[0].points.map(p=>`<text class="response-tick" x="${48+(p.severity/maxSeverity)*270}" y="190" text-anchor="middle">${pct(p.severity)}</text>`).join('');$('#response-plot').innerHTML=`<svg viewBox="0 0 360 210" role="img" aria-label="Four within-metric normalized response curves at three measured ${r.family} severity levels"><line class="response-grid" x1="48" y1="50" x2="326" y2="50"></line><line class="response-grid" x1="48" y1="108" x2="326" y2="108"></line><line class="response-axis" x1="48" y1="166" x2="326" y2="166"></line><line class="response-axis" x1="48" y1="40" x2="48" y2="166"></line><text class="response-tick" x="39" y="54" text-anchor="end">100%</text><text class="response-tick" x="39" y="112" text-anchor="end">50%</text><text class="response-tick" x="39" y="170" text-anchor="end">0</text>${series.map(s=>s.markup).join('')}${ticks}<text class="response-tick" x="187" y="207" text-anchor="middle">${r.severityLabel}</text></svg><div class="response-points">${series.map(s=>s.legend).join('')}</div>`;$('#response-family').textContent=`${r.family} family`;$('#response-title').textContent='Severity → metric response';$('#response-scope').textContent=`${r.series.length} metrics · ${r.aggregation}`;$('#response-caption').textContent=`${r.aggregation} at ${ordered[0].points.length} measured severity levels. Each metric is divided by its own observed maximum for shape comparison; lines connect observations only.`;}
function renderCalibration(d){$('#metric-cases').textContent=d.caseCount;$('#metric-candidates').textContent=d.candidateCount;$('#metric-records').textContent=d.recordCount;$('#metric-oracles').textContent=d.oraclesValid&&d.violationsDetected?'100%':'Check';$('#calibration-protocol').textContent=d.protocol;$('#calibration-source').textContent=d.source;renderResponse(d.severityResponse);const headers=['Candidate','Physics caught',...Object.values(d.metricLabels)];let html=headers.map(h=>`<div class="matrix-cell heading">${h}</div>`).join('');for(const row of d.rows){html+=`<div class="matrix-cell name">${row.name}</div>${valueCell(row.physicsDetection)}`;for(const key of Object.keys(d.metricLabels)){html+=valueCell(row.lowSensitivity[key]);}}$('#sensitivity-matrix').innerHTML=html;}
...
window.addEventListener('DOMContentLoaded',()=>{const menu=$('[data-view-menu]');$('[data-alvenx-home]').addEventListener('click',backToTop);load().catch(error=>{$('#run-status').textContent=error.message});const initial=location.hash==='#learned-view'?'learned':'calibration';show(initial,{scroll:false,updateHash:false});$$('[data-view]').forEach(button=>button.addEventListener('click',()=>show(button.dataset.view)));menu.addEventListener('keydown',event=>{if(event.key==='Escape'&&menu.open){menu.removeAttribute('open');menu.querySelector('summary').focus();}});document.addEventListener('click',event=>{if(menu.open&&!menu.contains(event.target))menu.removeAttribute('open');});window.addEventListener('hashchange',()=>{if(location.hash==='#learned-view'||location.hash==='#calibration-view')show(location.hash==='#learned-view'?'learned':'calibration',{scroll:true,updateHash:false});});$('#run-smoke').addEventListener('click',run);});
"""
